...
def load_maze_from_file(filename):
    ...
    if maze:
        ...
        for row in maze:
            while len(row) < max_cols: row.append('X')
    return maze
...
def look_ahead(maze, row, col, dir_idx):
    """Operacion 1 – Ver adelante: PARED, CAMINO o SALIDA."""
    dr, dc = DIRECTIONS[dir_idx]
    nr, nc = row + dr, col + dc
    if not (0 <= nr < len(maze) and 0 <= nc < len(maze[0])): return 'PARED'
...
def turn_left(dir_idx):
    """Operacion 3 – Girar 90° a la izquierda: N→O→S→E→N."""
    return (dir_idx + 3) % 4


def solve(maze, visited, path, path_set, log, pos, dir_idx, is_advance, verbose, step):
    row, col = pos
    state = (row, col, dir_idx)

    # Evitar ciclos dentro de la rama actual
    if state in visited:
        return False
    visited.add(state)
    ...
    if is_advance:
        if pos in path_set:
            if verbose: log.append(f"           [descarta {pos}: ya en camino activo]")
        ...
        if maze[row][col].upper() == 'S':
            ...
            if verbose:
                log.append(f"  Avance {step[0]:>3} │ prof={len(path):>3} │ {pos} dir={DIR_NAMES[dir_idx]} → SALIDA ✓")
            visited.discard(state)
            return True
        ...
        step[0] += 1
        if verbose:
            log.append(f"  Avance {step[0]:>3} │ prof={len(path):>3} │ {pos} dir={DIR_NAMES[dir_idx]}")
    else:
        if verbose:
            log.append(f"           Gira → ahora mira {DIR_NAMES[dir_idx]}")

    # Operacion 1: ver adelante
    ...
    if ahead != 'PARED':
        dr, dc = DIRECTIONS[dir_idx]
        if solve(maze, visited, path, path_set, log,
                 (row + dr, col + dc), dir_idx, True, verbose, step):
            visited.discard(state)
            return True

    # Operacion 3: girar izquierda
    if solve(maze, visited, path, path_set, log,
             pos, turn_left(dir_idx), False, verbose, step):
        visited.discard(state)
        return True

    # BACKTRACK: deshacer avance y liberar estado para otras ramas
    if is_advance:
        path.pop(); path_set.remove(pos)
        if verbose:
            parent = path[-1] if path else None
            log.append(f"  ───────── BACKTRACK: abandona {pos}, vuelve a {parent} (prof={len(path)})")
...
def run_maze(filename):
    ...
    if start is None:
        print(f"[{filename}] ERROR: no se encontro 'E'"); return None, rows, cols
    if end is None:
        print(f"[{filename}] ERROR: no se encontro 'S'"); return None, rows, cols
    ...
    for initial_dir in range(4):
        ...
        log.append(f"Orientacion inicial: {DIR_NAMES[initial_dir]}")
        log.append(f"  {'─'*55}")
        step = [0]
        if solve(maze, visited, path, path_set, log,
    ...
    print(f"\n--- Resultado ---")
    if found:
        print(f"Solucion: {len(path)} celdas  ({elapsed:.6f} s)")
        if verbose:
            print("Camino: " + " → ".join(str(p) for p in path))
            print("\nLaberinto con solucion ('.' = camino):")
            print_solution(maze, path)
        return elapsed, rows, cols
    else:
        print(f"Sin solucion  ({elapsed:.6f} s)")
        return None, rows, cols


def plot_times(labels, cell_counts, times, output_path="tiempos_backtracking.png"):
    if len(times) < 2:
        print("Se necesitan al menos 2 laberintos para graficar."); return
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(cell_counts, times, 'o-', color='steelblue', linewidth=2, markersize=8, label='Tiempo medido')
    scale = max(times) / max(cell_counts)
    ax.plot(cell_counts, [n * scale for n in cell_counts], '--', color='tomato',
            linewidth=1.5, label='Curva teorica O(m·n) [escalada]')
    for n, t, lbl in zip(cell_counts, times, labels):
        ax.annotate(lbl, (n, t), textcoords="offset points", xytext=(5, 4), fontsize=7)
    ax.set_xlabel("Tamaño (celdas = m × n)", fontsize=11)
    ax.set_ylabel("Tiempo de ejecucion (s)", fontsize=11)
    ax.set_title("Backtracking – Tiempo vs Tamaño\nComplejidad teorica: O(m·n)", fontsize=12)
...
if __name__ == '__main__':
    ...
    for filepath in sys.argv[1:]:
        if not os.path.isfile(filepath):
            print(f"ADVERTENCIA: no se encontro '{filepath}', se omite."); continue
    ...
    if times:
        ...
        print("\nNo se resolvio ningún laberinto correctamente.")